...
namespace Breakout {
	...
	*/
	...
	public:
		EnemyHead(Shape::Rectangle realm, std::shared_ptr<Moving> moving) {
			m_life = Life(4, 4);
			m_realm = realm;
			m_moving = moving;
			Object::layer = PRIORITY_DYNAMIC_OBJECT;
		...
		std::shared_ptr<Fireball> makeFireball(int mode, Eigen::Vector2f velocity) override {
			...
			auto fireball = std::make_shared<Fireball>(Shape::Circle(m_realm.getLeftBottomPoint(), FIREBALL_RADIUS), moving);
			return fireball;
		}
		bool damageEnemy(std::shared_ptr<Fireball>& fireball) override {
			...
			return m_life.damage(fireball->giveDamage());
		}
		void updatePosition() {
